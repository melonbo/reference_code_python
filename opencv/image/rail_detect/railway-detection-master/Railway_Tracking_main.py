...
def main():
    ...
    orb = cv2.ORB_create()#定位关键点
    bf = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True)#二维特征点匹配
    #step 3 创建目标检测器
    ...
    while(r == True):
        ...
        kp, des = orb.detectAndCompute(matchImage, None)#返回关键点信息及描述符
        #kp：关键点信息，包括位置，尺度，方向信息
        #des：关键点描述符，每个关键点BRIEF特征向量，二进制字符串

        # INIT HAAR CASCADE CLASSIFIERS 初始化分类器
        ...
        if not firstFrame:
            ...
            matches = bf.match(des_old, des)#返回最匹配的
