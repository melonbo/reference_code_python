...
def thresholding(img):
    ...
    x_thresh = utils.abs_sobel_thresh(img, orient='x', thresh_min=20 ,thresh_max=280)
...
def processing(img,M,Minv,left_line,right_line):
    ...
    print(type(img),type(thresholded), type(thresholded_wraped))
    print(img.shape, thresholded.shape, thresholded_wraped.shape)
    # cv2.imshow('1', cv2.resize(thresholded, None, fx=0.5, fy=0.5))
    # cv2.imshow('2', cv2.resize(thresholded_wraped, None, fx=0.5, fy=0.5))
    # cv2.waitKey(0)
...
def main02():
    ...
    # path = r"E:\work\pycode\util\opencv\picture\rail_detect\Rail_Lane_Lines_master\pic\385.jpg"
    # path = r"F:\share\video\qd3\light-detect\rail-car-day\rail-car-day-00250.jpg"
    # path = r"F:\share\image\rail-lane\f413543f79f5432d530273369213453a.jpeg"
    img = cv2.imread(path)
