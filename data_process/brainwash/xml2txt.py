...
def single_xml_to_txt(xml_file):
    ...
    with open(txt_file, 'w') as txt_file:
        for member in root.findall('object'):
            ...
            box_x_min = int(member[2][0].text)  # 左上角横坐标
            box_y_min = int(member[2][1].text)  # 左上角纵坐标
            box_x_max = int(member[2][2].text)  # 右下角横坐标
            box_y_max = int(member[2][3].text)  # 右下角纵坐标
            # 转成相对位置和宽高
